...
class ResourceDepositService
{
    ...
    public function charges(
        ...
        // A supplied allocation is authoritative, including optional/replacement staff.
        if ($selectedResourceQuantities !== null) {
            $involved = $quantities;
            // Holds also contain conditional candidates; an unanswered/negative
            // question does not mean those staff will attend.
            foreach ($type->questions as $question) {
                ...
                if ($rule !== null && ! $this->isTriggered($rule, $answers[$question->uuid] ?? null)) {
                    foreach ($rule->resources as $resource) {
                        unset($involved[$resource->getKey()]);
                    }
                }
            }
            if (Resource::query()->whereKey(array_keys($involved))->where('type', 'person')->exists()) {
                return [];
            }
        } else {
            // Without an allocation, only staff that are actually required can waive security.
            foreach ($this->requirements->requiredResources($type) as $resource) {
                if ($resource->type === 'person' && ! isset($conditionalResourceKeys[$resource->getKey()])) {
                    return [];
                }
            }
            foreach ($this->requirements->replacementGroups($type) as $group) {
                if ($group->isNotEmpty() && $group->every(fn (Resource $resource): bool => $resource->type === 'person')) {
                    return [];
                }
            }
            foreach ($type->questions as $question) {
                ...
                $hasStaff = $rule->fulfillment_mode === ConditionalResourceFulfillmentMode::OneOf
                    ? $rule->resources->isNotEmpty() && $rule->resources->every(fn (Resource $resource): bool => $resource->type === 'person')
                    : $rule->resources->contains('type', 'person');
                if ($hasStaff) {
                    return [];
                }
            }
