...
class BookingController extends Controller
{
    ...
    public function overrideDeposit(Request $request, Booking $booking, OrganizationContext $context): RedirectResponse
    {
        $this->sameOrganization($booking, $context);
        $this->authorize('manageScheduling', $context->organization());
        $data = $request->validate([
            'override_global_deposit' => ['required', 'boolean'],
            'deposit_override' => ['exclude_unless:override_global_deposit,1', 'required', new \App\Rules\MoneyAmount($booking->currency, true)],
        ]);
        $amount = $request->boolean('override_global_deposit')
            ? app(\App\Domain\Money\MoneyService::class)->parse($data['deposit_override'], $booking->currency)
            : null;
        app(\App\Domain\Resources\BookingDepositOverrideService::class)->apply($booking, $amount, $request->user()->uuid);

        return back()->with('success', 'Booking deposit updated.');
    }
