...
<div class="alert alert-info">
    Active organization holidays block every appointment type and resource in <strong>{{ $organization->timezone }}</strong>. They override weekly hours, custom schedules, and extra-availability exceptions. Country and region holidays remain optional until you add them below.
</div>
...
<div class="row g-4 mb-4">
    <div class="col-lg-5">
        <div class="card h-100">
            <div class="card-body">
                <h2 class="h4">Add a country or region holiday</h2>
                <p class="text-body-secondary">The timezone suggests a location, but the saved selection is explicit so travel, VPNs, and remote administrators do not change the calendar.</p>
                <form method="post" action="{{ route('availability.holidays.region.update') }}" class="mb-4">
                    @csrf @method('PATCH')
                    <label class="form-label" for="holiday-region">Country or region</label>
                    <div class="d-flex gap-2">
                        <select class="form-select" id="holiday-region" name="holiday_region" required>
                            @foreach($regionOptions as $code => $label)
                                <option value="{{ $code }}" @selected(old('holiday_region', $selectedRegion) === $code)>{{ $label }}</option>
                            @endforeach
                        </select>
                        <button class="btn btn-outline-primary" type="submit">Apply</button>
                    </div>
                    @if($suggestedRegion)
                        <div class="form-text">Suggested from {{ $organization->timezone }}: {{ $regionOptions[$suggestedRegion] }}.</div>
                    @else
                        <div class="form-text">This timezone does not identify one country. Choose the organization’s holiday jurisdiction.</div>
                    @endif
                </form>
                ...
                <form method="post" action="{{ route('availability.holidays.store') }}">
                    ...
                    <label class="form-label" for="provider-holiday-key">Available holiday</label>
                    <select class="form-select mb-3" id="provider-holiday-key" name="provider_holiday_key" required @disabled($selectedRegion === null || $regionalPresets === [])>
                        <option value="">Choose a holiday</option>
                        @foreach($regionalPresets as $preset)
                            <option value="{{ $preset['key'] }}" @selected(old('provider_holiday_key') === $preset['key'])>
                                {{ $preset['name'] }} — {{ collect($preset['dates'])->first() }}
                            </option>
                        @endforeach
                    </select>
                    @if($selectedRegion !== null && $regionalPresets === [])
                        <p class="text-body-secondary small">Every listed official or bank holiday for this region is already configured.</p>
                    @endif
                    <button class="btn btn-primary" type="submit" @disabled($selectedRegion === null || $regionalPresets === [])>Add closure</button>
                </form>
            </div>
        </div>
    </div>
