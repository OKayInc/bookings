...
class M9R6ResourceDepositTest extends TestCase
{
    ...
    public function test_allocated_staff_waive_all_equipment_deposits_but_unallocated_staff_do_not(): void
    {
        $organization = Organization::factory()->create(['currency' => 'CAD']);
        $type = $this->appointmentType($organization);
        $equipment = $this->equipment($organization, 'Projector', 2, 2500);
        $staff = Resource::create(['organization_id' => $organization->getKey(), 'name' => 'Operator', 'type' => 'person']);
        $type->resources()->attach($equipment->getKey(), ['requirement_mode' => 'required', 'is_required' => true]);
        $type->resources()->attach($staff->getKey(), ['requirement_mode' => 'optional', 'is_required' => false]);
        $service = app(ResourceDepositService::class);
        $this->assertSame(2500, $service->total($type->fresh()));
        $this->assertSame(2500, $service->total($type->fresh(), [], [$equipment->getKey() => 1]));
        $this->assertSame(0, $service->total($type->fresh(), [], [$equipment->getKey() => 1, $staff->getKey() => 1]));
        $type->resources()->updateExistingPivot($staff->getKey(), ['requirement_mode' => 'required', 'is_required' => true]);
        $this->assertSame(0, $service->total($type->fresh()));
    }

    public function test_conditional_staff_only_waive_deposits_when_triggered(): void
    {
        $organization = Organization::factory()->create(['currency' => 'CAD']);
        $type = $this->appointmentType($organization);
        $equipment = $this->equipment($organization, 'Projector', 2, 2500);
        ...
        $type->resources()->attach($staff->getKey(), ['requirement_mode' => 'optional', 'is_required' => false]);
        $question = $this->conditionalQuestion($type);
        $question->resourceRequirementRule->resources()->attach($staff->getKey());
        $service = app(ResourceDepositService::class);
        $this->assertSame(2500, $service->total($type->fresh()));
        $this->assertSame(0, $service->total($type->fresh(), [$question->uuid => $question->resourceRequirementRule->triggerOption->uuid]));
        $allocation = [$equipment->getKey() => 1, $staff->getKey() => 1];
        $this->assertSame(2500, $service->total($type->fresh(), [], $allocation));
        $this->assertSame(0, $service->total($type->fresh(), [$question->uuid => $question->resourceRequirementRule->triggerOption->uuid], $allocation));
    }

    public function test_staff_can_override_zero_increase_and_restore_before_payment(): void
    {
        [$user, $organization] = $this->organizationContext();
        [$booking, $payment] = $this->paidDepositBooking($organization);
        $payment->delete();
        $booking->update(['paid_minor' => 0, 'subtotal_minor' => 12000, 'payment_status' => 'unpaid', 'status' => 'pending_payment']);
        $booking->resourceDeposits()->create([
            'resource_name' => 'Kit', 'quantity' => 1, 'unit_amount_minor' => 2000,
            'amount_minor' => 2000, 'currency' => 'CAD', 'configuration_source' => 'resource_default',
        ]);
        $booking->priceLines()->create([
            'source_type' => 'resource_deposit', 'label' => 'Kit deposit', 'line_type' => 'resource_deposit',
            'quantity' => '1', 'amount_minor' => 2000, 'position' => 1,
        ]);
        $this->actingAs($user)->withSession(['active_organization_uuid' => $organization->uuid]);
        $url = route('bookings.deposit-override', $booking);
        $this->post($url, ['override_global_deposit' => '1', 'deposit_override' => '0'])->assertSessionHasNoErrors();
        $booking->refresh();
        $this->assertSame(0, $booking->deposit_override_minor);
        $this->assertSame(0, $booking->deposit_minor);
        $this->assertSame(10000, $booking->price_minor);
        $this->assertSame(2500, $booking->initial_payment_due_minor);
        $this->post($url, ['override_global_deposit' => '1', 'deposit_override' => '50.00'])->assertSessionHasNoErrors();
        $this->assertSame(15000, $booking->fresh()->price_minor);
        $this->assertSame(7500, $booking->fresh()->initial_payment_due_minor);
        $this->post($url, ['override_global_deposit' => '0'])->assertSessionHasNoErrors();
        $booking->refresh();
        $this->assertNull($booking->deposit_override_minor);
        $this->assertSame(2000, $booking->deposit_minor);
        $this->assertSame(12000, $booking->price_minor);
        $this->assertSame('Kit', $booking->resourceDeposits()->first()->resource_name);
        $this->post($url, ['override_global_deposit' => '1', 'deposit_override' => '-1'])->assertSessionHasErrors('deposit_override');
    }

    public function test_deposit_override_rejects_collected_payment_and_other_organizations(): void
    {
        [$user, $organization] = $this->organizationContext();
        [$booking] = $this->paidDepositBooking($organization);
        $this->actingAs($user)->withSession(['active_organization_uuid' => $organization->uuid])
            ->post(route('bookings.deposit-override', $booking), ['override_global_deposit' => '1', 'deposit_override' => '0'])
            ->assertSessionHasErrors('deposit_override');
        $this->assertSame(2000, $booking->fresh()->deposit_minor);
        [$otherUser, $otherOrganization] = $this->organizationContext();
        $this->actingAs($otherUser)->withSession(['active_organization_uuid' => $otherOrganization->uuid])
            ->post(route('bookings.deposit-override', $booking), ['override_global_deposit' => '1', 'deposit_override' => '0'])
            ->assertNotFound();
    }
