@php
    $isPersonResource = old('type', $resource?->type ?? 'person') === 'person';
@endphp
<div class="row">
...
<div class="field" id="resource-person-field" @if(! $isPersonResource) hidden @endif>
<label>Linked organization member (optional)</label>
<select id="resource-person" name="person_uuid" @disabled(! $isPersonResource)><option value="">None</option>@foreach($members as $member)<option value="{{ $member->uuid }}" @selected(old('person_uuid', $selectedPersonUuid) === $member->uuid)>{{ $member->full_name }} — {{ $member->user?->email ?: $member->primary_email }} — {{ ucfirst((string) $member->pivot->role) }}</option>@endforeach</select>
...
<div class="field" id="resource-timezone-field" @if(! $isPersonResource) hidden @endif><label>Timezone override (optional)</label><select id="resource-timezone" name="timezone" @disabled(! $isPersonResource)><option value="">Use organization timezone</option>@foreach($timezones as $timezone)<option value="{{ $timezone }}" @selected(old('timezone', $resource?->timezone) === $timezone)>{{ $timezone }}</option>@endforeach</select></div>
...
<div class="field" id="resource-holiday-field" @if(! $isPersonResource) hidden @endif>
<label>Holiday calendar</label>
<div class="checkbox-list"><label><input id="enforce-resource-holidays" type="checkbox" name="enforce_holidays" value="1" @checked(old('enforce_holidays', $resourceHolidayEnforced)) @disabled(! $isPersonResource)> Enforce official and bank holidays for this resource</label></div>
<select id="resource-holiday-region" name="holiday_region" @disabled(! $isPersonResource)>
...
(() => {
    ...
    const personField = document.getElementById('resource-person-field');
    const personSelect = document.getElementById('resource-person');
    const timezoneField = document.getElementById('resource-timezone-field');
    const timezone = document.getElementById('resource-timezone');
    const holidayField = document.getElementById('resource-holiday-field');
    ...
    if (type) {
        const refreshPersonSettings = () => {
            const isPerson = type.value === 'person';
            [personField, timezoneField, holidayField].forEach(field => {
                if (field) field.hidden = !isPerson;
            });
            [personSelect, timezone, enforce, region].forEach(control => {
                if (control) control.disabled = !isPerson;
            });
            if (region && enforce) region.required = isPerson && enforce.checked;
        };
        type.addEventListener('change', refreshPersonSettings);
        if (enforce) enforce.addEventListener('change', refreshPersonSettings);
        refreshPersonSettings();
    }
