...
@case('number') <input id="q_{{ $question->uuid }}" type="number" name="{{ $key }}" value="{{ old($oldKey) }}" @if(isset($cfg['min'])) min="{{ $cfg['min'] }}" @endif @if(isset($cfg['max'])) max="{{ $cfg['max'] }}" @endif step="{{ $cfg['step'] ?? (in_array($question->pricing_adjustment_type->value,['none','rate'],true) ? 'any' : '1') }}" @required($question->is_required)>
@if($question->pricing_adjustment_type->value==='rate')<div class="muted">Price add-on: answer × {{ $money->format((int)$question->pricing_amount_minor,$organization->currency) }} after {{ $question->pricing_included_units }} included unit(s).</div>@elseif($question->pricing_adjustment_type->value!=='none')<div class="muted">This answer affects the price. {{ $question->pricing_included_units }} unit(s) included.</div>@endif @break
...
<div class="section-card" id="questionnaire-price-card" @if(!($showPricing ?? true)) hidden @endif>
