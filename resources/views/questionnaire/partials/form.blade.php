...
<div class="field"><label for="question-description">Description / help text</label><textarea id="question-description" name="description" rows="8" data-rich-text-editor>{{ old('description',$question?->description) }}</textarea><p class="muted">Only character formatting is retained. Links, media, colors, font sizes, and external content are removed.</p></div>
...
<input type="hidden" name="resource_requirement_enabled" value="0">
<label class="inline-check"><input id="resource-requirement-enabled" type="checkbox" name="resource_requirement_enabled" value="1" @checked($resourceRequirementEnabled)> Change optional resources to required based on this answer</label>
<p class="muted">The question stays in its normal questionnaire position. If the selected time cannot satisfy this group, the question is hidden and the configured unavailable answer is saved automatically.</p>
<div id="resource-requirement-fields" style="margin-top:1rem">
<div class="row three">
 <div class="field"><label for="resource-requirement-trigger">Answer that requires resources</label><select id="resource-requirement-trigger" name="resource_requirement_trigger_option_index" required><option value="">Choose an answer…</option>@foreach($rows as $i=>$o)<option value="{{ $i }}" @selected((string)$resourceTriggerIndex === (string)$i)>{{ $o['label'] ?? 'Option '.($i+1) }}</option>@endforeach</select></div>
 <div class="field"><label for="resource-unavailable-default">Answer when unavailable</label><select id="resource-unavailable-default" name="resource_unavailable_default_option_index" required><option value="">Choose an answer…</option>@foreach($rows as $i=>$o)<option value="{{ $i }}" @selected((string)$resourceDefaultIndex === (string)$i)>{{ $o['label'] ?? 'Option '.($i+1) }}</option>@endforeach</select><div class="muted">For a Yes/No question, normally choose No.</div></div>
 <div class="field"><label for="resource-requirement-group-name">Resource group name</label><input id="resource-requirement-group-name" name="resource_requirement_group_name" maxlength="80" value="{{ $resourceGroupName }}" placeholder="For example: Video" required></div>
</div>
...
(function(){
 ...
 const resourceRequirementEnabled=document.getElementById('resource-requirement-enabled'); const resourceRequirementFields=document.getElementById('resource-requirement-fields'); const resourceTrigger=document.getElementById('resource-requirement-trigger'); const resourceDefault=document.getElementById('resource-unavailable-default');
 ...
 function toggleResourceRequirement(){if(!resourceRequirementEnabled||!resourceRequirementFields)return;const enabled=['checkboxes','radio','select'].includes(type.value)&&resourceRequirementEnabled.checked;resourceRequirementFields.style.display=enabled?'block':'none';resourceRequirementFields.querySelectorAll('input,select,textarea').forEach(c=>{c.disabled=!enabled;});}
 function refreshResourceAnswerOptions(){if(!resourceTrigger||!resourceDefault)return;const currentTrigger=resourceTrigger.value,currentDefault=resourceDefault.value;const options=Array.from(document.querySelectorAll('.option-row')).map((row,position)=>({index:row.dataset.optionIndex,label:row.querySelector('input[name$="[label]"]')?.value.trim()||`Option ${position+1}`}));[resourceTrigger,resourceDefault].forEach((select,selectIndex)=>{const selected=selectIndex===0?currentTrigger:currentDefault;select.innerHTML='<option value="">Choose an answer…</option>'+options.map(option=>`<option value="${option.index}" ${option.index===selected?'selected':''}>${escape(option.label)}</option>`).join('');});}
 ...
 if(resourceRequirementEnabled)resourceRequirementEnabled.addEventListener('change',toggleResourceRequirement);if(rows)rows.addEventListener('input',e=>{if(e.target.matches('input[name$="[label]"]'))refreshResourceAnswerOptions();});
 document.addEventListener('click',e=>{if(e.target.classList.contains('remove-option')){e.target.closest('.option-row').remove();refreshResourceAnswerOptions();}if(e.target.classList.contains('remove-distance-range'))e.target.closest('.distance-range-row').remove();if(e.target.classList.contains('remove-visibility-condition')){e.target.closest('.visibility-condition-row').remove();refreshConditionRows();}});
})();
</script>
