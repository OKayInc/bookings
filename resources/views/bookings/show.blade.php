...
<div class="card">
    ...
    <p>Staff allocated to a new booking automatically waive resource deposits. A manual override replaces the entire deposit for this booking.</p>
    @if($booking->deposit_override_minor !== null)
        <p>Current override: <strong>{{ $money->format($booking->deposit_override_minor, $booking->currency) }}</strong></p>
    @endif
    @if(! $booking->payments->count() && ! $booking->paid_minor && ! $booking->refunded_minor && ! in_array($booking->status, [\App\Enums\BookingStatus::Cancelled, \App\Enums\BookingStatus::Declined], true))
    <form method="post" action="{{ route('bookings.deposit-override', $booking) }}" data-deposit-override-form>
        @csrf
        <input type="hidden" name="override_global_deposit" value="0">
        <label><input type="checkbox" name="override_global_deposit" value="1" data-deposit-override-toggle @checked(old('override_global_deposit', $booking->deposit_override_minor !== null))> Override global deposit</label>
        <div class="field" data-deposit-override-fields>
            <label for="deposit_override">Total deposit ({{ $booking->currency }})</label>
            <input id="deposit_override" name="deposit_override" inputmode="decimal"  value="{{ old('deposit_override', $money->decimal($booking->deposit_override_minor ?? $booking->deposit_minor, $booking->currency)) }}">
            <p class="muted">Enter zero to waive the deposit. Uncheck to restore the original booking deposit.</p>
        </div>
    ...
    @else
        <p class="muted">Deposit settings are locked after payment starts or the booking closes. Use the deposit refund action to return a collected deposit.</p>
    @endif
</div>
@endif
...
@push('scripts')
<script>
document.querySelectorAll('[data-deposit-override-form]').forEach((form) => {
    const toggle = form.querySelector('[data-deposit-override-toggle]');
    const fields = form.querySelector('[data-deposit-override-fields]');
    const amount = fields.querySelector('input');
    const sync = () => {
