...
class PercentageService {
    public function parseToBasisPoints(string|int|null $value): ?int {
        ...
        $significantWhole=ltrim($w,'0');
        if (strlen($significantWhole)>4 || (strlen($significantWhole)===4 && strcmp($significantWhole,'1000')>0)) throw new InvalidArgumentException('Percentage cannot exceed 1000%.');
        $bps=((int)$w*100)+(int)$f;
