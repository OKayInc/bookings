...
- **M5 Questionnaire + Pricing — complete:** unlimited and reusable organization questions, chargeable checkbox/select/number answers, files, email/phone/address verification and deterministic price snapshots.
- **M6 Confirmation + Policies — this package:** employee/resource acceptance, cancellation/rescheduling policy enforcement and reminders.
- **M7 Calendars:** Google Calendar and Microsoft Graph availability/synchronization with per-appointment-type calendar selection.
- **M8 Payments:** organization-specific Stripe/PayPal, retainers, balances, refunds, whitelist/blacklist rules.
- **M9 Administration:** reporting, audit history, operational tooling and additional health checks.
