...
M9 makes payment a tenant-owned workflow: every organization supplies its own encrypted merchant credentials and every booking keeps immutable collection/refund terms. M9-R1 adds globally shared physical equipment stock with organization-currency pricing on each appointment-type assignment; M9-R6 adds refundable resource deposits with immutable booking snapshots and provider-bound refunds. See `docs/PAYMENTS.md`, `docs/EQUIPMENT-INVENTORY.md`, and `docs/UPGRADE-M9-R5-TO-M9-R6.md`.
